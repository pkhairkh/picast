...
The boGDan browser extension is a Manifest V3 extension for Chrome and Firefox that enables users to cast web page URLs directly from the browser to a boGDan receiver on the local network. This document specifies the manifest.json structure, permissions, background service worker, content scripts, and the discovery mechanism.
...
```json
{
  ...
  "name": "boGDan",
  "version": "0.1.0",
  "description": "Cast web videos to boGDan receiver through Tor",
...
| `storage` | Persist extension settings (boGDan device IP, quality preference) | Stores the last-known boGDan device address and user preferences across browser sessions. |
| `dns` | mDNS service discovery on the local network | Used to discover boGDan devices advertising `_bogcast._tcp` via mDNS/DNS-SD without requiring the user to manually enter the IP address. |
...
| `declarativeNetRequest` (optional) | Modify request headers declaratively | Future use for adding custom headers to boGDan requests. Manifest V3 requires declarative rules instead of blocking webRequest. |
...
1. **mDNS Discovery**: Periodically queries `_bogcast._tcp` to discover boGDan devices on the local network.
2. **Cast Requests**: When the user clicks "Cast", the service worker sends a `POST /api/v1/cast` request to the discovered boGDan device.
3. **WebSocket Connection**: Maintains a WebSocket connection to the boGDan device for real-time status updates (playback state, position, errors).
...
Extension installed / browser started:
  ...
  2. Discover boGDan devices via mDNS
  3. If device found: connect WebSocket, update badge
  4. If no device: show "No boGDan found" badge
...
|------|-----------------|---------------|
| Direct `<video src="">` | `HTMLMediaElement.src` property | Send URL directly to boGDan HTTP API |
| `<video>` with `<source>` children | `HTMLSourceElement.src` | Send source URL to boGDan |
| MSE (MediaSource Extensions) | `MediaSource.readyState` property | Intercept network requests (requires `webRequest` permission) |
| YouTube embed | `#movie_player` element + `ytplayer` API | Extract video ID, construct YouTube URL, send to boGDan |
| Vimeo embed | `iframe[src*="vimeo.com"]` | Extract video ID from iframe src |
| HLS manifest | `.m3u8` URL in network requests | Send manifest URL directly to boGDan |
...
|---------|------|---------|-------------|
| boGDan Address | string | (auto-discovered) | Manual IP address override if mDNS discovery fails |
...
│       ├── mDNS discovery     ← Discovers boGDan devices on LAN
