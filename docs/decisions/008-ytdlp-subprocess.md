...
PiCast receives media URLs from users via the HTTP API, browser extension, or DLNA. Many of these URLs are not direct media links — they point to web pages that embed video players (YouTube, Vimeo, Twitch, etc.). PiCast needs to resolve these page URLs into direct media stream URLs that GStreamer can play.
...
PiCast must decide how to integrate yt-dlp:
...
PiCast runs yt-dlp as a subprocess. The `picast-resolver` crate spawns `yt-dlp` with the following configuration:
...
- **`--dump-json`**: Outputs a JSON object with `url`, `format_id`, `height`, `vcodec`, and other fields. PiCast parses this to get the direct media URL.
- **`--no-download`**: Only extract metadata, don't download.
- **`--socket-timeout 30`**: 30-second timeout for network operations. If yt-dlp hangs (e.g., Tor circuit is slow), PiCast kills the subprocess after 30 seconds.
...
| Outcome | Impact |
|---------|--------|
| ✅ Process isolation | If yt-dlp crashes (Python exception, OOM, segfault), it doesn't affect the PiCast process; the resolver just returns an error and PiCast continues running |
| ✅ Independent updates | yt-dlp can be updated independently via `pip install --upgrade yt-dlp` without rebuilding PiCast; site extractors change frequently and independent updates are critical |
...
| ❌ Python startup overhead | yt-dlp is a Python script; cold start takes 5–15 seconds on Pi 4 due to Python interpreter + yt-dlp module loading. Mitigated by: (a) PiCast keeps a "warm" resolver by pre-fetching URL info, (b) the HTTP API shows a "resolving" status to the user during this time |
| ❌ Subprocess management | Must handle process spawning, timeout enforcement, stdout capture, and error parsing; adds ~200 lines of Rust code to `picast-resolver` |
| ❌ Python dependency | yt-dlp requires Python 3.7+ on the Pi; PiCast OS image must include Python (~30 MB) and yt-dlp pip package (~10 MB) |
...
|-------------|---------------------|
| **Embedded Python via pyo3** | Embedding a Python interpreter adds ~50 MB RAM overhead permanently; pyo3 FFI bridge is complex; Python GIL limits concurrency; yt-dlp crashes inside the embedded interpreter would crash PiCast (no process isolation); harder to update yt-dlp independently |
