...
- **Severity:** High
- **Location:** Lines 412–423 (`/api/cast` handler — `uuid::Uuid::new_v4()` at line 412, `CastResponse` construction at line 423)
...
- **Severity:** Medium
- **Location:** Lines 458–463 (`/api/seek` handler — handler at line 458, `unwrap_or(0)` at line 463)
...
- **Severity:** Medium
- **Location:** Lines 390–416 (`/api/cast` handler — `current_status()` check at line 390, `tokio::spawn` at line 416)
...
- **Severity:** Medium
- **Location:** Lines 282–295 (`extract_client_ip` — function at line 282, `"unknown".to_owned()` return at line 295)
...
- **Location:** Lines 532–537 (`json_response` — function at line 532, `Access-Control-Allow-Origin: *` header at line 537), also in `cors_response` (line 586) and `rate_limit_response` (line 595)
...
- **Severity:** Medium
- **Location:** Lines 282–290 (`extract_client_ip` — function at line 282, X-Forwarded-For parsing at lines 283–290)
...
- **Severity:** Low
- **Location:** Lines 490–500 (`/api/audio-device` POST handler — handler at line 490, `sink_type` check at line 496)
- **Description:** The `AudioDeviceRequest.sink_type` field defaults to `"alsasink"`. The handler at line 496 checks `if payload.sink_type != "alsasink"` but does **not** reject invalid values — instead it calls `session.set_audio_sink(payload.sink_type.clone())` with the arbitrary string, only logging a warning on failure (line 498). A client could set `sink_type` to any string (e.g. `"wrongtsink"` or a nonexistent GStreamer element) and the API returns 200 OK even though audio playback will fail silently or with a confusing GStreamer error.
...
- **Severity:** Low
- **Location:** Lines 428–454 (stop at line 428, pause at line 446, resume at line 452 — all use `error_response(StatusCode::CONFLICT, ...)`)
- **Description:** The `stop`, `pause`, and `resume` endpoints return 409 CONFLICT for any error from the session manager, including "no active session" (which is not a conflict — it's a not-found condition). The `error_response` function (line 558–562) maps `StatusCode::CONFLICT => ErrorCode::SessionActive`, which is misleading when the actual error is `NoActiveSession`.
...
- **Severity:** Low
- **Location:** Lines 458 (`/api/seek`), 471 (`/api/volume`), 490 (`/api/audio-device`) — all use `read_body_json(...).await?` instead of explicit error handling
...
- **Severity:** Medium
- **Location:** Lines 699–850 (`list_alsa_devices` — function at line 699, `pactl` subprocess at line 714/730, `bluetoothctl` subprocess at line 787)
...
- **Severity:** Low
- **Location:** Lines 106–111 (`#[allow(dead_code)]` at line 106, `enum ErrorCode` at line 107, `NoActiveSession` variant at line 111)
- **Description:** The `ErrorCode::NoActiveSession` variant is defined at line 111. The `ErrorCode` enum itself is marked `#[allow(dead_code)]` at line 106 ("Some variants reserved for future use"), but it is never used in any response. The volume handler (line 481) matches on `bogdan_session::SessionError::NoActiveSession` but maps it to `StatusCode::CONFLICT`, which `error_response` (line 562) then maps to `ErrorCode::SessionActive`. The variant is only referenced in a test (line 967), never in a real response.
...
- **Severity:** Low
- **Location:** Line 45 (`struct CastRequest` — only has `url` field)
...
- **Severity:** Low
- **Location:** Lines 1020–1024 (`empty_body_rejected` test)
